...
// AccountView
    ...
    /***************************************************************************
     * ShowInstitutionDepartmentListView **************************************
     **************************************************************************/
    Views.ShowInstitutionDepartmentListView = Views.BaseView.extend({
        ...
        initialize: function (options) {
            ...
            self.template = _.template(tpl_institution_department_list);
            self.institutions = new Models.Institutions();
            self.schools  = new Models.Schools();
            self.departments = new Models.Departments();

            // re-render the view in case model has been successfully saved
            self.institutions.bind("onSaved", this.render, this);
            self.schools.bind("onSaved", this.render, this);
            self.departments.bind("onSaved", this.render, this);

        },

        events: {
            "click a#editInstitution": "edit",
            "click a#editSchool": "edit",
            "click a#editDepartment": "edit"
        },
            ...
            self.institutions.fetch({
                ...
                success: function (model, response, options) {
                    ...
                    $select.chosen().change(function () {
                            if ($select.val() !== '') {
                                self.$("a#editInstitution").attr("disabled", false);
                                self.renderSchools($select.val());
                            } else {
                                self.$("a#editInstitution").attr("disabled", true);
                            }
                        }
                    );
                }
        ...
        renderSchools: function (institutionId) {

            var self = this;

            self.schools.fetch({
                url: self.institutions.url + '/' + institutionId + '/schools',
                cache: false,
                wait: true,
                success: function (model, response, options) {
                    var $select = self.$("select[name=school]");
                    $select.empty();
                    $select.append('<option value="">' + $.i18n.prop("chooseSchool") + '</option>');
                    self.schools.each(function (school) {
                        // re-assign the default url
                        school.url = new Models.School().url;
                        $select.append('<option value="' + school.get('id') + '">' + school.getName(App.locale) + '</option>');
                    });

                    self.$("select[name=school]").attr("disabled", false);
                    $select.trigger("chosen:updated");

                    $select.chosen().change(function () {
                            if ($select.val() !== '') {
                                self.renderDepartments($select.val());
                                self.$("a#editSchool").attr("disabled", false);
                            } else {
                                self.$("a#editSchool").attr("disabled", true);
                            }
                        }
                    );
                }
        ...
        renderDepartments: function (schoolId) {

            var self = this;

            self.departments.fetch({
                url: self.schools.url + '/' + schoolId + '/departments',
                cache: false,
                wait: true,
                success: function (model, response, options) {
                    var $select = self.$("select[name=department]");
                    $select.empty();
                    $select.append('<option value="">' + $.i18n.prop("chooseDepartment") + '</option>');
                    self.departments.each(function (department) {
                        $select.append('<option value="' + department.get('id') + '">' + department.getName(App.locale) + '</option>');
                        // re-assign the defailt url
                        department.url = new Models.Department().url;
                    });

                    self.$("select[name=department]").attr("disabled", false);
                    $select.trigger("chosen:updated");

                    $select.chosen().change(function () {
                            if ($select.val() !== '') {
                                self.$("a#editDepartment").attr("disabled", false);
                            } else {
                                self.$("a#editDepartment").attr("disabled", true);
                            }
                        }
                    );
                }
        ...
        initDropDownLists: function () {
            ...
            self.$("a#editInstitution").attr("disabled", true);
            self.$("select[name=institution]").chosen({width: "25%"});
            self.$("select[name=institution]").append('<option value="">'+$.i18n.prop("chooseInstitution")+'</option>');
            self.$("select[name=institution]").trigger("chosen:updated");

            self.$("select[name=school]").attr("disabled", true);
            self.$("a#editSchool").attr("disabled", true);
            self.$("select[name=school]").chosen({width: "25%"});
            self.$("select[name=school]").append('<option value="">' + $.i18n.prop("chooseSchool") + '</option>');
            self.$("select[name=school]").trigger("chosen:updated");

            self.$("select[name=department]").attr("disabled", true);
            self.$("a#editDepartment").attr("disabled", true);
            self.$("select[name=department]").chosen({width: "25%"});
            self.$("select[name=department]").append('<option value="">' + $.i18n.prop("chooseDepartment") + '</option>');
            self.$("select[name=department]").trigger("chosen:updated");
        },
        ...
        edit: function (event) {
            ...
            var edit = $(event.currentTarget).attr('id');

            var modelToUpdate;

            if (edit === 'editInstitution') {
                modelToUpdate = self.institutions.get(self.$("select[name=institution]").val());
            } else if (edit === 'editSchool') {
                modelToUpdate = self.schools.get(self.$("select[name=school]").val());
            } else if (edit === 'editDepartment') {
                modelToUpdate = self.departments.get(self.$("select[name=department]").val());
            }

            var view = new Views.ShowUpdateInstitutionAndDepartmentNamesView({
                        model: modelToUpdate
            });

            self.$("div#editForm").html(view.el);
        ...
        close: function () {
            ...
            $(this.el).unbind();
            $(this.el).remove();
            this.institutions.unbind("onSaved", this.render, this);
            this.schools.unbind("onSaved", this.render, this);
            this.departments.unbind("onSaved", this.render, this);
        }
    });

    /***************************************************************************
     * ShowUpdateInstitutionAndDepartmentNamesView *****************************
     **************************************************************************/
    Views.ShowUpdateInstitutionAndDepartmentNamesView = Views.BaseView.extend({
        tagName: "div",
        ...
        initialize: function (options) {
            ...
            self.template = _.template(tpl_institution_department_edit);
        },
        ...
        render: function () {
            ...
            if (self.model instanceof Models.Institution) {
                type = 'institution';
            } else if (self.model instanceof Models.School) {
                type = 'school';
            } else if (self.model instanceof Models.Department) {
                type = 'department';
            }

            var tpl_data = {
                object : self.model.toJSON(),
                type : type
            };
        ...
        submit: function (event) {
            ...
            // save temp the names
            var prevName = self.model.get("name");

            // get the new values inserted by the user
            var el_name = self.$("form input[name=el_"+ self.model.get("id") +"]").val();
            var en_name = self.$("form input[name=en_"+ self.model.get("id") +"]").val();
