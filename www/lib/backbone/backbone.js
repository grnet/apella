...
(function() {
	...
	var Events = Backbone.Events = {
		...
		// the
		// event. If `events` is null, removes all bound callbacks for all
		// events.
		...
		trigger : function(events) {
			...
			// first to trigger the event, then to trigger any `"all"`
			// callbacks.
			while (event = events.shift()) {
				if (node = calls[event]) {
					tail = node.tail;
	...
	_.extend(Model.prototype, Events, {
		...
		// A hash of attributes that have changed since the last `'change'`
		// event
		// began.
		_pending : null,
		...
		set : function(key, value, options) {
			...
			for (attr in attrs) {
				...
				// If the new and previous value differ, record the change. If
				// not,
		...
		// Remove an attribute from the model, firing `"change"` unless you
		// choose
		...
		// Clear all attributes on the model, firing `"change"` unless you
		// choose
		// to silence it.
		...
		change : function(options) {
			...
			// Continue firing `"change"` events while there are pending
			// changes.
		...
		// Check if the model is currently in a valid state. It's only possible
		// to
	...
	_.extend(Collection.prototype, Events, {
		...
		add : function(models, options) {
			...
			// Begin by turning bare objects into model references, and
			// preventing
			...
			// Insert models into the collection, re-sorting if needed, and
			// triggering
		...
		// Remove a model, or a list of models from the set. Pass silent to
		// avoid
